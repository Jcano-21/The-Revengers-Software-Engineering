...
def get_resupply_dates(consumables, data):
      # Set up the SQLAlchemy engine
        username = 'root'
        password = 'Th3RevengersTe4m'
        db_name = 'barrios'
        ...
        #    print('Current category: ', category)
            # Query the 'resupply' table for each category
        query = f"""
            SELECT
                *
            FROM
        ...
        print('category data: ', category_data)
            #category_data['start_date'] = pd.to_datetime(category_data['start_date'])
            #category_data['end_date'] = pd.to_datetime(category_data['end_date'])
            # Iterate through the rows of the DataFrame
        
        consumables.load_resupply_dates(category_data)
        return